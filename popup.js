...
function createCard(project, index) {
  ...
  mycard.innerHTML = `
      <img src='${project.featuredImg}' alt='Recent Work'>
      <div class='works-card-main'>
          ...
          <div class='d-flex works-card-client'>
              ...
              <p class='my-works-card-client-role paragraph'>${project.details.profile}</p>
              <i class='fa-solid fa-circle my-works-card-client-counter'></i>
              <p class='my-works-card-client-year paragraph'>${project.details.year}</p>
          </div>
          <p class='paragraph works-card-paragraph'>${project.Description}</p>
          <ul class="works-card-skills">
              ${project.language.map((skill) => `<li class="skill-list">${skill}</li>`).join('')}
          </ul>
        ...
        <div class='d-flex card-detail-header'>
            <div>
                ...
                <div class='d-flex works-card-client'>
                    <p class='paragraph'>${project.details.company}</p>
                    <i class='fa-solid fa-circle my-works-card-client-counter'></i>
                    <p class='works-card-client-role paragraph'>${project.details.profile}</p>
                    <i class='fa-solid fa-circle my-works-card-client-counter'></i>
                    <p class='my-works-card-client-year paragraph'>${project.details.year}</p>
                </div>
            </div>
            <i id='card-detail-close-${index + 1}' onclick="hideProjectDetails(${index + 1})" class='fa-solid fa-xmark card-detail-cross-icon'></i>
        </div>
        ...
        <div class='card-detail-body'>
            ...
            <div class='card-detail-body-right'>
                <ul class="d-flex card-detail-body-tags">
                  ${project.language.map((skill) => `<li class="card-detail-body-tag">${skill}</li>`).join('')}
                </ul>
                <div class='card-detail-body-right-button-container'>
                    <button class='card-detail-button' src='${project.seeLive}'>
...
window.onload = function LoadPopup() {
  ...
  projects.forEach((project, index) => {
    ...
    popupContainer.appendChild(createPopup(project, index));
  });
};